"""
GAT Baseline Integration Module

Training and prediction functions for the standard GAT baseline model.
Mirrors the interface of dag_transformer_integration.py for consistency.
"""
...
def train_gat_baseline(
    ...
    """
    Training function for GAT Baseline
    ...
    Args:
        ...
        dag: DAG structure (used for node info only)
        random_seed: Random seed
        dag_attention_mask: Whether to use DAG-based attention masking (ignored for GAT, always False)
    ...
    print(f"[GAT Baseline] Using device: {device}")
    ...
    wandb.init(project="GAT Baseline", entity="mliu7", config=config)
    ...
    for epoch in range(train_config["n_epochs"]):
        ...
        for batch_ix, (batch_raw, batch_binned) in enumerate(train_dataloader):
            ...
            # Note: mask parameter is ignored by GAT (always fully-connected)
            outputs = model(batch, mask=False, estimator=estimator)
        ...
        with torch.no_grad():
            ...
            for batch_raw_val, batch_binned_val in val_dataloader:
                ...
                outputs_val = model(batch_val, mask=False, estimator=estimator)
                
                if estimator == "g-formula":
                    y = batch_val['y'].to(device).float()
                    y_ = torch.squeeze(outputs_val['y']).to(device).float()
                    val_batch_loss, _ = g_formula_loss_fun(y_, y)
                elif estimator == "ipw":
                    t = batch_val['t'].to(device).float()
                    e = outputs_val['t'].to(device).squeeze().float()
                    val_batch_loss, _ = ipw_loss_fun(e, t)
                else:  # aipw
                    y = batch_val['y'].to(device).float()
                    y_ = torch.squeeze(outputs_val['y']).to(device).float()
                    t = batch_val['t'].to(device).float()
                    e = outputs_val['t'].to(device).squeeze().float()
                    val_batch_loss, _ = aipw_loss_fun(y_, y, e, t)
        ...
        predictions, metrics_val = predict_gat_baseline(
            model=model,
            data_name=data_name,
            data=val_data,
        ...
        if eval_metric == "both":
            print(
                f"[GAT] Epoch {epoch + 1}/{train_config['n_epochs']}, "
                f"Test ATE: {test_ate:.4f}, Test NRMSE: {test_nrmse:.4f}, "
                f"Test MAE: {test_mae:.4f} ± {test_mae_std:.4f}")
        elif eval_metric == "mae":
            print(
                f"[GAT] Epoch {epoch + 1}/{train_config['n_epochs']}, "
                f"Test ATE: {test_ate:.4f}, Test MAE: {test_mae:.4f} ± {test_mae_std:.4f}")
        else:  # nrmse
            print(
                f"[GAT] Epoch {epoch + 1}/{train_config['n_epochs']}, "
                f"Test ATE: {test_ate:.4f}, Test NRMSE: {test_nrmse:.4f}")
...
def predict_gat_baseline(
    ...
    """
    Prediction function for GAT Baseline
    ...
    if estimator == "g-formula":
        predictions_y0 = predict_gat_outputs(model, dataloader_A0, estimator, dag_attention_mask)['y']
        predictions_y0 = pd.DataFrame(predictions_y0, columns=['pred_y_A0'])
        
        predictions_y1 = predict_gat_outputs(model, dataloader_A1, estimator, dag_attention_mask)['y']
        ...
        predictions_t = predict_gat_outputs(model, dataloader, estimator, dag_attention_mask)['t']
...
def predict_gat_outputs(model, dataloader, estimator, dag_attention_mask: bool = False):
    """
    Helper function to generate GAT model predictions
    ...
    with torch.no_grad():
        for batch_raw, _ in dataloader:
            batch = {k: v.to(device) for k, v in batch_raw.items()}
            # GAT always uses mask=False (fully-connected attention)
            outputs = model(batch, mask=False, estimator=estimator)
